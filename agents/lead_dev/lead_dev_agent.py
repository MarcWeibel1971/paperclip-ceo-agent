...
def main():
    log("Lead Developer Agent started.")
    ...
    commented = 0
    system_prompt = """Du bist der Lead Fullstack Developer von Pensionierung Plus (pensionierung-plus.ch).
Deine Aufgaben umfassen:
- Architektur-Entscheidungen und Code-Reviews
- Behebung von komplexen Bugs (z.B. PDF-Extraktion, GPT-4 Vision)
- Umsetzung von Features gemäss Spezifikation

Analysiere das zugewiesene Issue aus Entwickler-Sicht.
Gib einen klaren technischen Lösungsansatz, nenne betroffene Dateien oder Module und schlage konkrete Code-Anpassungen vor.
...
"""
    ...
    for issue in assigned_issues[:2]:
        ...
        user_prompt = f"""Bitte analysiere folgendes Issue aus Entwickler-Sicht:
...
"""
        ...
        if analysis:
            comment_body = (
                ...
                f"{analysis}\n\n"
                f"*Status: In Bearbeitung. Ich werde die vorgeschlagenen Code-Änderungen prüfen.*"
            )
            if post_comment(issue_id, comment_body):
                log(f"Posted Lead Developer analysis on {issue.get('identifier', issue_id)}")
